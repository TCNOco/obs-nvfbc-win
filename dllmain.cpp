...
static bool create_capture_session(data_nvfbc_t* data_nvfbc, obs_data_t* settings)
{
	...
	uint32_t dwOutput_id = obs_data_get_int(settings, "screen");
	uint32_t dwSampling_rate_ms = 1000.0 / obs_data_get_int(settings, "fps") + 0.5;
	NVFBC_CREATE_CAPTURE_SESSION_PARAMS cap_params = {
		...
		.dwSamplingRateMs = dwSampling_rate_ms,
...
#define GL_TEXTURE_MAX_LEVEL 0x813D
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 0); // This was missing for some reason -- GL_TEXTURE_MAX_LEVEL
...
static bool update_texture(data_t* data)
{
	...
	if (!data->nvfbc.has_capture_session || !switch_to_nvfbc_context(&data->nvfbc)) {
...
static void* create(obs_data_t* settings, obs_source_t* source)
{
	...
	data_t* data = static_cast<data_t*>(bzalloc(sizeof(data_t)));
	if (data == NULL) {
...
{
	data_t* data = static_cast<data_t*>(p);
